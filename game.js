...
var bgm = new Audio("map.mp3");    //background music
bgm.volume = 0.8;
...
function keydown(evt) {
    ...
    switch (keyCode) {
        ...
        case "C".charCodeAt(0):
                ...
                svgdoc.getElementById("player").style.setProperty("fill-opacity", 0.5);;
                cheatmode = true;
            break;
            
        case "V".charCodeAt(0):
                svgdoc.getElementById("cheat").style.setProperty("visibility", "hidden", null);
                svgdoc.getElementById("player").style.setProperty("fill-opacity", 1);;
                cheatmode = false;
            break;
    }
}
...
function collisionDetection() {
    ...
    if(intersect(new Point(560,220),new Size(10,40), player.position, PLAYER_SIZE))
    {
        var portal = new Audio("port.wav");
        portal.play();
        ...
        if(bullet.getAttribute("id")!="monBullet"){
            ...
            for (var i = 0; i < monsters.childNodes.length; i++) {
                ...
                if(intersect(mon,MONSTER_SIZE, bul, BULLET_SIZE)){
                    var mdead = new Audio("monster.wav");
                    mdead.play();
    ...
    for (var i = 0; i < flowers.childNodes.length; i++) {
        ...
        if (intersect(new Point(x,y), FLOWER_SIZE, player.position, PLAYER_SIZE) ) {
            ...
            var getflower = new Audio("getflower.mp3");
            getflower.volume = 0.6;
            getflower.play();
        }
    }
    ...
    if (intersect(new Point(540,300), new Size(10,40), player.position, PLAYER_SIZE) && gameclear)
    {
        var lvUp = new Audio("levelup.mp3");
        lvUp.play();
        nextLevel();
    }
...
function updateScreen() {
    ...
    svgdoc.getElementById("nameBar").setAttribute("y", player.position.y - 5);
...
function gameover(){
    bgm.pause();
    bgm.currentTime = 0;
    var dead = new Audio("gameover.mp3");
    dead.play();
    clearInterval(gameInterval);
    clearTimeout(timeInterval);
    clearTimeout(monShootTimeout);
